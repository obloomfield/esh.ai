...
def train_vae(model, train_loader, args, is_cvae=False):
    ...
    for batch in train_loader:
        ...
        with tf.GradientTape() as tape:
            if is_cvae:
                (x_hat, mu, logvar) = model.call(batchdata,one_hot(c,class_size=10))
            else:
                (x_hat, mu, logvar) = model.call(batchdata) #mnist is tupled by lages
            loss = loss_function(x_hat, batchdata, mu, logvar)
            total_loss += loss
